...
1. Поднимите PostgreSQL (например, через `docker compose up -d`).
2. Создайте и заполните `.env` по примеру `.env_example`.
...
4. Для входа используйте любого тестового пользователя (пароль для всех: `password`):
   - `alex@example.com`
   - `maria@example.com`
   - `oleg@example.com`
